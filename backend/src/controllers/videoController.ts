...
import { presignTryOnJobs } from '../services/imageUrlService';
...
import { sanitizeTryOnTitle, TRYON_STORAGE_LIMIT } from './tryonController';
...
// Resolve ONE video source — a camera-roll `file`, a completed try-on
// (`sourceJobId`), or a profile body photo (`bodyPhoto`) — into a tryon-videos/
...
async function resolveVideoSource(
  ...
  if (sourceJobId) {
    const job = await prisma.tryOnJob.findFirst({
      ...
      select: { resultFullBodyUrl: true, resultMediumUrl: true, videoUrl: true },
    });
    const src = job?.resultFullBodyUrl ?? job?.resultMediumUrl ?? null;
    if (!job || job.videoUrl || !src) {
      return {
        ok: false,
        status: 400,
        error: 'INVALID_SOURCE',
        message: "That try-on can't be used as a video source.",
      };
    }
    const key = await copyWithinS3(keyFromUrl(src), 'tryon-videos', userId, `${uuidv4()}-src.jpg`);
    return { ok: true, key };
  }
  ...
  return {
    ...
    status: 400,
    error: 'NO_SOURCE',
    message: 'Pick a photo, a try-on, or a body photo to animate.',
  };
}
...
// `photo2` (camera roll), `sourceJobId` / `sourceJobId2` (a completed try-on's
// result), or `bodyPhoto` / `bodyPhoto2` = 'full'|'medium' (a profile body
// photo). Chosen images are copied/uploaded under tryon-videos/ so the job owns
// the keys. The primary lands in bodyPhotoUrl (poster); the optional second in
// clothingPhoto1Url and is passed to Grok as a `reference_images` entry, with
...
export async function submitVideo(req: Request, res: Response): Promise<void> {
  ...
  // (keeps the Guideline 1.2 moderation surface small), matching guest try-ons.
  const isPrivate =
  ...
  // Same explicit AI-processing consent gate as try-on (we send a user image to
  ...
  // Storage cap (shared with try-on — both are tryon_jobs rows).
  const storedCount = await prisma.tryOnJob.count({ where: { userId, status: { not: 'FAILED' } } });
  if (storedCount >= TRYON_STORAGE_LIMIT) {
    res.status(403).json({
      error: 'TRYON_LIMIT_REACHED',
      message: `You've reached the ${TRYON_STORAGE_LIMIT}-session storage limit. Delete some from your Profile to continue.`,
    });
    return;
  }
  ...
  // utils/videoSource.test.ts), then resolve each into a tryon-videos/ key the
  ...
  if (!primary) {
    res
      .status(400)
      .json({ error: 'NO_SOURCE', message: 'Pick a photo, a try-on, or a body photo to animate.' });
    return;
  }
  ...
  // Same soft per-user throttle as try-on (parity): bursts beyond the tier free
  ...
  // Counts all non-FAILED tryon_jobs (images + videos share the window) and
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.tryOnJob.create({
        data: {
          ...
          clothingPhoto1Url: secondImageKey,
          perspectivesUsed: [],
