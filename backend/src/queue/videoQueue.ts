...
// AI Video (image-to-video) generation queue. Separate from the try-on queue
// because video is a single async Grok call with polling (not the multi-
// perspective image pipeline), but it writes back to the SAME tryon_jobs row
...
export async function enqueueVideo(data: VideoJobData, delayMs = 0): Promise<void> {
  await videoQueue.add('process', data, {
    ...
    // Age-based (see tryonQueue): keep failed jobs ≤7 days / ≤50 most recent so
