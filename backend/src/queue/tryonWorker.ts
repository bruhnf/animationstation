...
function alertAdminsOfTryOnFailure(data: {
  jobId: string;
  userId?: string | null;
  ...
  void Promise.allSettled(
    ...
    if (failed > 0) {
      log.error('Some try-on failure alert emails failed to send', { jobId: data.jobId, failed });
    } else {
      log.info('Try-on failure alert emailed to admins', { jobId: data.jobId, kind: data.kind });
    }
  });
}
...
const worker = new Worker<TryOnJobData>(
  ...
  async (job) => {
    ...
    const existing = await prisma.tryOnJob.findUnique({
      where: { id: jobId },
      select: { resultFullBodyUrl: true, resultMediumUrl: true },
    });
    const existingKeyFor = (p: 'full_body' | 'medium'): string | null =>
      (p === 'full_body' ? existing?.resultFullBodyUrl : existing?.resultMediumUrl) ?? null;
    ...
    async function generateAndPersist(
      ...
      try {
        ...
        if (genErr instanceof ContentModeratedError) {
          ...
          log.warn('Try-on blocked by content moderation', {
            jobId,
            userId,
            perspective: bodyPhoto.perspective,
...
worker.on('failed', async (job, err) => {
  ...
  if (isTerminal && !isModerated) {
    Sentry.captureException(err, {
      tags: { service: 'queue', queue: 'tryon' },
      extra: {
        jobId: job?.data?.jobId,
        userId: job?.data?.userId,
        attemptsMade,
  ...
  if (isModerated) {
    ...
    log.warn(
      withinGrace
        ? 'Try-on blocked by content moderation — warning issued, credit refunded'
        : 'Try-on blocked by content moderation — credit NOT refunded (ToS 5.4)',
...
// Refund the credit a job deducted at submit time, if any. tryonController
...
async function refundJobCredit(jobId: string, userId: string): Promise<boolean> {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      // hardcoded 1, so the refund always matches the charge if the per-try-on
