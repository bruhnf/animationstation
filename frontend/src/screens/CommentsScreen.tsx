...
import { Comment, Creation } from '../types';
...
type Nav = NativeStackNavigationProp<RootStackParams, 'Comments'>;
type Rt = RouteProp<RootStackParams, 'Comments'>;

interface JobWithUser extends Creation {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [jobRes, commentsRes] = await Promise.all([
        api.get<JobWithUser>(`/creations/${jobId}`),
        api.get<{ comments: Comment[] }>(`/creations/${jobId}/comments`),
      ]);
      setJob(jobRes.data);
