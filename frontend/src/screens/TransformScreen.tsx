...
// Keep in sync with CREATION_TITLE_MAX_LENGTH in backend creationsController — the
...
export default function TransformScreen() {
  ...
  // exclusive with clothingPhotos (one clothing source per creation).
  ...
  // image on the public feed. Capped to match the server (TITLE_MAX).
  ...
  const maxItems = 1; // One clothing item per creation
  ...
  async function handleSubmit() {
    ...
    await performTransformSubmit();
  }

  async function performTransformSubmit() {
    setSubmitting(true);
    try {
      ...
      } else if (error?.error === 'CREATION_LIMIT_REACHED') {
        Alert.alert(
          'Storage Full',
          error.message ??
        ...
        if (status === 429) {
          ...
          Alert.alert(
            ...
            [
              ...
              { text: 'Start Over', onPress: resetTransform, style: 'destructive' },
            ],
          );
        }
      }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
          ...
          <>
            ...
            <View style={styles.captionBlock}>
              ...
              <Text style={styles.captionHint}>
                Shown under your result on the feed. {TITLE_MAX - title.length} left
              </Text>
            </View>
        ...
        {activeJob && <ResultView job={activeJob} onReset={resetTransform} />}
