...
export default function CreationResultCard({ job, onPress }: Props) {
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.85}>
      <View style={styles.imageWrap}>
        ...
        {job.refImage1Url && (
          <RetryableImage uri={job.refImage1Url} style={styles.clothingThumb} resizeMode="cover" />
        )}
      </View>
...
function StatusBadge({ status }: { status: Creation['status'] }) {
  const colors: Record<Creation['status'], string> = {
